...
if not PINECONE_API_KEY:
    raise ValueError(
        "PINECONE_API_KEY is not set in environment variables or .env file")
...
def get_pinecone_index(index_name):
    """Get a Pinecone index, creating it if it doesn't exist"""
    ...
    try:
        ...
        print(f"Error creating index with serverless spec: {e}")
        ...
        try:
            ...
            print(f"Index {index_name} created with pod-based settings.")
        except Exception as e2:
            print(f"Failed to create index: {e2}")
            print(
                "Please create an index manually in the Pinecone console and try again.")
            raise
...
def upsert_data(index, df):
    """Upsert data to a Pinecone index"""
